---
title: "Switch AI Models and Hermes Profiles in VS Code"
...
The model dropdown auto-populates from Hermes via ACP. The extension first requests the model list from the `model.options` endpoint; if that returns nothing, it falls back to `session/set_model` from the native session API. The result is a grouped list that refreshes each time you connect.
...
If Hermes is not running or does not expose models via ACP, the extension falls back to a static list you define in your VS Code settings. Configure it by adding the following to your `settings.json`:
...
The `hermes.defaultModel` value must match an `id` from the `hermes.models` array. The fallback list is only used when ACP does not return a model catalog; it is replaced by the live list as soon as a connection is established.
