---
title: "Hermes Agent VS Code Extension Changelog"
sidebarTitle: "Changelog"
description: "Release notes for FTR10 Hermes VSCode — what's new, what's fixed, and what's changed in each version."
---
...
- **More robust content parsing.** Assistant and thought chunks now use `extractTextFromContentBlock` for reliable text extraction from ACP content blocks, fixing cases where partial or structured content blocks produced garbled output.
...
- **Profile-isolated storage.** Session history and model preferences are now stored per profile via `profileStorage`, so switching profiles gives you a clean slate for both chat history and model selection.
- Hermes profile CLI arguments, ACP model catalog parsing, and grouped model display (`acpModelCatalog`, `hermesProfile`, `profileDiscovery`).
